...
impl McpApps {
    ...
    pub fn is_empty(&self) -> bool {
        !self.claude && !self.codex && !self.gemini && !self.opencode && !self.hermes && !self.mimocode
    }
}
...
pub struct McpRoot {
    ...
    /// MiMoCode MCP 配置（实际使用 mimocode.json）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, claude-desktop, codex, gemini, opencode, openclaw, hermes, mimocode。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, claude-desktop, codex, gemini, opencode, openclaw, hermes, mimocode."),
            )),
        }
    }
}
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::MiMoCode => continue, // MiMoCode didn't exist in v3.6.x, skip
            };
